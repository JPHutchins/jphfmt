...
operands after an assignment or a `return`, and those of a `{}` element, a call
argument or an expression statement, are one expression, and parenthesizing one
expression changes nothing but the layout. A call's *only* argument is already
bounded by the call's own parentheses, so it gains no second pair. A span holding a depth-zero `,` is a
